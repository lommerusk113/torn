...
export class WarTracker {
  ...
  private async handleTracking(): Promise<void> {
    ...
    const data: WarMember[] = members.map((member: MemberWithId) => {
      const current: WarMember | undefined = storedMembers.find((x: WarMember) => x.member_id == member.id);
      const location = this.getLocation(member.status.description, member.status.state, current?.location);
  ...
  private getLocation(description: string, state: string, current?: Location): Location {
  ...
  private mapTornData(factionData: Record<string, FactionMember>): MemberWithId[] {
